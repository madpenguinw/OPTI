...
def main():
    ...
    for current_stage in range(1, j + 1):
        ...
        R_v = 287100  # Дж /(кг * К)
        k_v = 1.39  # точно
        C_pg = k_g / (k_g - 1) * R_g  # НАШЕЛ ОШИБКУ
        C_pv = k_g / (k_v - 1) * R_v
        ...
        print('alfa_0_grad =', alfa_0_grad)
        print('alfa_1_grad =', alfa_1_grad)
        ...
        print('ПРОВЕРЬ НОРМАЛЬНОСТЬ ДЗЕТ')
        ...
        fi_1 = m.sqrt(1 - dzeta_1)  # ЗАКОММЕНТИТЬ!!!!!
        print('ПРОВЕРЬ fi_1')
        print('fi_1 =', fi_1)
        ...
        W_2_t = m.sqrt(2 * rho_t * h_0 * T_1 / T_1_t + (W_1 ** 2) - (U_1 ** 2) + (U_2 ** 2))  # ИСПРАВИЛ ОШИБКУ
        ...
        if current_stage == j:
            ...
            print('P_w_2_t_z =', P_w_2_t_z)
            # print('((T_2_t / T_w_2_z) ** (k_g / (k_g - 1)) =', ((T_2_t / T_w_2_z) ** (k_g / (k_g - 1))))
            # print('T_2_t =', T_2_t)
            # print('T_w_2_z =', T_w_2_z)
            # print('k_g =', k_g)
        ...
        dzeta_2 = dzeta_2_pr + dzeta_2_vt + G_v_2_otn * ((1 - C_2_v_otn) ** 2) + Y_rk_sr  # ИСПРАВИЛ ОШИБКУ
        ...
        delta_eq = ((d_2_2sh / ((mi_oc * d_1_2sh * delta_1) ** 2) + Z_y / ((mi_r * k_y_2sh * delta_z_2sh) ** 2))) ** (-0.5)  # ИСПРАВИЛ ОШИБКУ
        ...
        # h_0_z = C_pg * (T_0_z - T_2_t_t_z)
        # Используется только в 71 формуле

        # 71) Внутренний КПД ступени по параметрам торможения
        # kpd_v_z = kpd_u * kpd_v / (h_0_z * kpd_u)
        # Данная величина нигде далее не учитывается
        ...
        sigma_delta_z = 0.97  # задается, коэффициент потерь полного давления в патрубке за ступенью
